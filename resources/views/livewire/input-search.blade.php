<div class="flex flex-col justify-center items-center">
    ...
    <form id="form_search" class="flex justify-center items-center w-[90%] 2xl:w-[1000px] mt-10 mb-10 gap-2" wire:submit="searchImages">
        <input 
            type="text" 
            name="text" 
            autocomplete="off" 
            ...
            maxlength="50" 
            wire:model="text" 
            class="drop-shadow-lg rounded-full h-16 flex-grow-2 px-8" />
    ...
    @if(count($images) > 0)
        <div class="grid-masonry">
            ...
            @foreach ($images as $image)
                <div class="grid-item mt-4 img"><img src="{{ $image['url'] }}" alt="{{ $image['alt'] }}" class="rounded-md drop-shadow-md hover:scale-105 hover:z-10 hover:border-white" /></div>
            @endforeach
        </div>
    @endif
